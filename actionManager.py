...
def add_message(medium='', date='', sender='', text='', fromUser=False, phone='null'):
    ...
    if medium == VM_medium or medium == CALLS_medium:
        if medium == VM_medium:
            text = 'Databases/Voicemails/' + text
...
def add_event(name, date):
    ...
    write_json(mydata, filename=GENERAL_EVENT_DB)


def remove_event(name, date):
    with open(GENERAL_EVENT_DB, 'r+') as file:
        data = json.load(file)
        rk = 0
        for evt in data['events']:
            if evt['name_event'] == name and evt['date'] == date:
                del data['events'][rk]
                break
            rk += 1
        file.close()
    with open(GENERAL_EVENT_DB, 'w') as data_file:
...
class ActionManagerObject:
    ...
    def lookup_event(self):
        return find_event()

    def add_event(self, name, date):
        add_event(name, date)

    def rm_event(self, name, date):
        remove_event(name, date)
...
if __name__ == "__main__":
    ...
    # add_event("tofind", "15/01/2022, 20:00:00")
    # print(find_event())
    # remove_event("tofind", "15/01/2022, 20:00:00")
    pass
